...
#define ICON0				COL1 "" COL0 /* frozen */
#define ICON1				COL1 "" COL0 /* MAILSYNC started */
#define ICON2				COL1 "" COL0 /* syncing */
        ...
        if (frozen == 2)
                ...
                if (n < 0)
                        *str = '\0';
                        ...
                        snprintf(str, CMDLENGTH, "%s%d", ICON0, n);
                frozen = 2;
                return;
        }
        if (sigval != NILL) {
                if (sigval > 0) {
                        ...
                        if (syncing && sigval == 1) {
                                snprintf(str, CMDLENGTH, "%s%d", ICON2, n);
                                return;
                        }
                } else {
                        if (n < 0) {
                ...
                switch (sigval) {
                        /* MAILSYNC started */
                        case 1:
                                snprintf(str, CMDLENGTH, "%s%d", ICON1, n);
                                break;
                        /* sync started */
                        case 2:
                                syncing = 1;
                        ...
                        case 3:
                                syncing = 0;
                                snprintf(str, CMDLENGTH, "%s%d", ICON3, n);
                                break;
                        /* sync failed */
                        case 4:
                                syncing = 0;
